...
public sealed class TiposCambioController(TipoCambioService s) : Controller
{
    ...
    [HttpGet("{id:guid}/editar")] public async Task<IActionResult> Editar(Guid id, CancellationToken c) { var t = await s.ObtenerAsync(id, c); return t is null ? NotFound() : View(new TipoCambioViewModel { CrcPorUsd = t.CrcPorUsd, Activo = t.Activo }); }
    ...
    [HttpPost("{id:guid}/eliminar")][ValidateAntiForgeryToken] public async Task<IActionResult> Eliminar(Guid id, CancellationToken c) { try { await s.EliminarAsync(id, c); } catch (InvalidOperationException e) { TempData["Error"] = e.Message; } return RedirectToAction(nameof(Index)); }
}
