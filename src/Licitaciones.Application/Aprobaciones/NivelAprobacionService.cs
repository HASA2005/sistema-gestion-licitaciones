...
public sealed class NivelAprobacionService(INivelAprobacionRepository repo)
{
    ...
    private static void ValidarTraslape(NivelAprobacion n, IEnumerable<NivelAprobacion> otros) { foreach (var x in otros) if ((n.MontoMaximoCrc is null || x.MontoMinimoCrc <= n.MontoMaximoCrc) && (x.MontoMaximoCrc is null || n.MontoMinimoCrc <= x.MontoMaximoCrc)) throw new InvalidOperationException("El rango se traslapa con otro nivel."); }
}
